...
> ✅ **โค้ด driver เสร็จแล้ว** — [lib/storage.ts](../../lib/storage.ts) `saveToSupabase()` ใช้ **`fetch` ไป Storage REST ตรง ๆ ไม่ต้องลง `@supabase/supabase-js`** (upload ท่าเดียว fetch พอ — ไม่เพิ่ม dependency). แผนเดิมให้ลง SDK — ของจริงไม่ต้อง

**ต้องทำ (infra) เฉพาะเมื่อจะใช้ `STORAGE_DRIVER=supabase`** (= deploy บน Vercel/serverless ที่เขียน disk ไม่ได้):
...
> **VPS ปัจจุบันใช้ `STORAGE_DRIVER=local`** (disk persistent) → **ไม่ต้องทำ 1c เลย**. 1c จำเป็นเฉพาะตอน deploy Vercel
...
### ขั้นตอน
1. Push branch ขึ้น GitHub → Vercel → **Import Project** → เลือก repo
2. **หน้า import = ปล่อย default หมด**: Preset `Next.js` (auto), Root `./`, **Build/Output/Install command อย่าเปิด toggle** — `postinstall: prisma generate` มีใน package.json แล้ว (gen `lib/generated/prisma` ตอน install เอง) ไม่ต้องตั้ง build command
3. **Environment Variables** (ตั้งใน Vercel, ทั้ง Production + Preview) — **นี่คือสิ่งเดียวที่ต้องตั้งจริง**:
   | key | ค่า |
   |---|---|
