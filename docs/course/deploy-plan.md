...
### เป้าหมาย
เปิดแอปด้วย domain ตัวเอง (เช่น `mycoffee.com`) แทน `*.vercel.app` + HTTPS

### ขั้นตอน
1. มี domain (ซื้อจาก Cloudflare/Namecheap/ฯลฯ)
2. Vercel → Project → **Settings → Domains** → Add `mycoffee.com` + `www.mycoffee.com`
3. ตั้ง **DNS records** ตามที่ Vercel บอก:
   - apex (`mycoffee.com`) → **A record** `76.76.21.21` (หรือชี้ nameserver ไป Vercel)
   - `www` → **CNAME** `cname.vercel-dns.com`
4. รอ DNS propagate (นาที–ชั่วโมง) → Vercel ออก **HTTPS (Let's Encrypt) อัตโนมัติ**
5. ตั้ง primary domain + redirect (`www` ↔ apex เลือกอันเดียวเป็นหลัก)

### teaching points
- **A vs CNAME** — apex ใช้ A (ชี้ IP), subdomain ใช้ CNAME (ชี้ชื่ออื่น); ทำไม apex ใส่ CNAME ตรง ๆ ไม่ได้
- **TLS อัตโนมัติ** — ไม่ต้องซื้อ cert เอง, Vercel ต่อ Let's Encrypt ให้; https ทั้งเว็บ → cookie `secure` โอเค
- ถ้าโค้ดมี absolute URL / `metadata.metadataBase` → ตั้งเป็น domain จริง (ตอนนี้ layout ยังไม่มี — เพิ่มถ้าจำเป็นตอนทำ SEO/OG image)
...
## นอกสโคป (ไว้ทีหลัง)
- CI/CD รัน `migrate deploy` อัตโนมัติตอน merge (ตอนนี้ทำ manual)
